<?php
/*
    Programm Preistracker (Docker Version)
...
function startup()
{
    global $docker_call, $docker_vz, $pdat_data; 
    $data = $docker_vz."/".$pdat_data;
    $os_bef = "";
    if (file_exists($docker_vz))
    {
        ...
        shell_exec($os_bef);        
        $os_bef = "sudo chmod 666 ".$pdat_data;
        shell_exec($os_bef);        
        $os_bef = "sudo chown www-data:www-data ".$pdat_data;
        shell_exec($os_bef);         
        $docker_call = 1;
        if (!file_exists($data))
        {
            $os_bef = "sudo cp ".$pdat_data." ".$data;
            shell_exec($os_bef);
        }
...
?>
    ...
    <html lang="de">
      ...
      <script> 
        function st_suche()
        {
          window.location.href = 'index.php?p=' + plz.value + '&w=' + prod.value;
        }
